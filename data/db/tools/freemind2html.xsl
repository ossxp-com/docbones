...
<xsl:param name="freemind_src">xs.mm</xsl:param>
...
<head>
	...
	<xsl:element name="link">
		...
		<xsl:attribute name="href">
			<xsl:value-of select="$document_root" /><xsl:value-of select="$freemind_dir_css"/>docbook.css</xsl:attribute>
		<xsl:attribute name="type">text/css</xsl:attribute>
	</xsl:element>
	<xsl:element name="link">
		<xsl:attribute name="rel">stylesheet</xsl:attribute>
		<xsl:attribute name="href">
			<xsl:value-of select="$document_root" /><xsl:value-of select="$freemind_dir_css"/>freemind2html.css</xsl:attribute>
		<xsl:attribute name="type">text/css</xsl:attribute>
	</xsl:element>
	...
	<xsl:element name="script">
		...
		<xsl:attribute name="src">
			<xsl:value-of select="$document_root" /><xsl:value-of select="$freemind_dir_js"/>freemind2html.js</xsl:attribute>
	</xsl:element>
	-->
          ...
          <![CDATA[
               function toggle(id)
               {
                   ...
                   if (div_el.style.display != 'none')
                   {
                      ...
                      img_el.src = '<xsl:value-of select="$document_root" /><xsl:value-of select="$freemind_dir_img"/>show.png';
          <![CDATA[
                   }
                   else
                   {
                      ...
                      img_el.src = '<xsl:value-of select="$document_root" /><xsl:value-of select="$freemind_dir_img"/>hide.png';
          <![CDATA[
                   };
               };
          ]]>
...
<body>
	...
	<xsl:element name="script"><xsl:attribute name="type">text/javascript</xsl:attribute>
	  <xsl:attribute name="src"><xsl:value-of select="$document_root" /><xsl:value-of select="$freemind_dir_js"/>header.js</xsl:attribute>
	</xsl:element>
	<xsl:element name="script"><xsl:attribute name="type">text/javascript</xsl:attribute>
	  write_header("<xsl:value-of select="$document_root" />");
	</xsl:element>
	...
	<!-- place Flash -->
	<xsl:element name="script"><xsl:attribute name="type">text/javascript</xsl:attribute>
	<xsl:attribute name="src"><xsl:value-of select="$document_root" /><xsl:value-of select="$freemind_dir_flash"/>mmflash.js</xsl:attribute>
	</xsl:element>
...
<![CDATA[
		...
		if (hasReqestedVersion) {
			FlashObject(
				...
				"movie", "<xsl:value-of select="$document_root" />/includes/mmflash/visorFreemind.swf",
<![CDATA[
				"width", "100%",
				"height", "500",
				"align", "middle",
				...
				"initLoadFile", "<xsl:value-of select="$freemind_src" />",
<![CDATA[
	...
	<xsl:element name="noscript">
		<xsl:element name="div"><xsl:attribute name="id">flashcontent</xsl:attribute>
			Flash plugin or Javascript are turned off.
			Activate both and reload to view the mindmap.<br/>
	...
	<!-- place footer -->
	<xsl:element name="script"><xsl:attribute name="type">text/javascript</xsl:attribute>
	  <xsl:attribute name="src"><xsl:value-of select="$document_root" /><xsl:value-of select="$freemind_dir_js"/>footer.js</xsl:attribute>
	</xsl:element>
	<xsl:element name="script"><xsl:attribute name="type">text/javascript</xsl:attribute>
	  write_footer("<xsl:value-of select="$document_root" />");
	</xsl:element>
</body>
...
<div>
	...
	<xsl:when test="node">
		...
		<xsl:element name="img">
			<xsl:attribute name="src">
				<xsl:value-of select="$document_root" /><xsl:value-of select="$freemind_dir_img"/>hide.png</xsl:attribute>
	...
	<xsl:otherwise>
		...
		<xsl:element name="img">
			<xsl:attribute name="src">
				<xsl:value-of select="$document_root" /><xsl:value-of select="$freemind_dir_img"/>leaf.png</xsl:attribute>
			<xsl:attribute name="class">hideshow</xsl:attribute>
	...
	<xsl:when test="not($show_link_url='true')">
		...
		<xsl:if test="not($show_icons='false')">
			...
			<xsl:element name="a">
				...
				<xsl:element name="img">
					<xsl:attribute name="src"><xsl:value-of select="$document_root" /><xsl:value-of select="$freemind_dir_img"/>ilink.png</xsl:attribute>
...
<xsl:template name="output-icons">
	<xsl:if test="not($show_icons='false')">
		<xsl:for-each select="icon">
			<xsl:element name="img">
				<xsl:attribute name="src">
					<xsl:value-of select="$document_root" /><xsl:value-of select="$freemind_dir_img"/>icons/<xsl:value-of select="@BUILTIN" />.png</xsl:attribute>
